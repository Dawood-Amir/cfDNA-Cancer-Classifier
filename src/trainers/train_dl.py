...
from models.residual_ffn import ResidualFFN
...
from  trainers.label_smoothing_cross_entropy import LabelSmoothingCrossEntropy 
...
def train_dl_model( model: nn.Module, config:yaml, device ,train_loader , val_loader  ):
    ...
    model_cfg = config["models"][config["model"]["type"]]
    loss_type = model_cfg.get("loss_type", "focal")  # default to focal
    smoothing = model_cfg.get("smoothing", 0.1)

    if loss_type == "label_smoothing":
        criterion = LabelSmoothingCrossEntropy(smoothing=smoothing)
    else:  # focal
        class_weights_tensor = None  # or compute if needed
...
def build_model(config, device):
    ...
    elif model_type == "resffn":
        return ResidualFFN(config).to(device)
